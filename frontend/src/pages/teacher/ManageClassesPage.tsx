...
// Giả lập danh sách buổi học theo ID môn học
...
const ManageClassesPage: React.FC = () => {
  ...
  // State để theo dõi Môn học nào đang được chọn. Nếu null nghĩa là đang ở màn hình Danh sách Môn học.
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 text-slate-800 font-sans">
      <SidebarTeacher activeItem="Manage Classes" />

      <main className="ml-[270px] flex-1 p-8 md:p-12 overflow-y-auto relative z-0">
        {/* --- TRẠNG THÁI 1: HIỂN THỊ DANH SÁCH MÔN HỌC (CLASSES) --- */}
        {!selectedClass && (
        ...
        {/* --- TRẠNG THÁI 2: HIỂN THỊ DANH SÁCH BUỔI HỌC CỦA MÔN ĐƯỢC CHỌN (SESSIONS) --- */}
        {selectedClass && (
          <div className="animate-in fade-in slide-in-from-right-8 duration-300">
            {/* Header của View Sessions */}
              ...
              {mockSessionsByClassId[selectedClass.id]?.length === 0 ? (
                ...
                <div className="flex flex-col gap-4">
                  {mockSessionsByClassId[selectedClass.id].map((session) => (
                    <div
                      key={session.id}
                      className="group flex flex-col md:flex-row md:items-center justify-between p-6 bg-slate-50 hover:bg-blue-50/50 rounded-2xl border border-slate-100 hover:border-blue-100 transition-colors gap-4"
                    >
                      {/* Session Info */}
                      <div className="flex items-center gap-5">
                        <div
                          className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${session.status === "Upcoming" ? "bg-blue-600 text-white" : "bg-white border border-slate-200 text-slate-400"}`}
                        >
                          <Video size={20} />
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-4 text-xs font-semibold text-slate-500 mt-1">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] uppercase tracking-widest ${session.status === "Upcoming" ? "bg-blue-100 text-blue-700" : "bg-slate-200 text-slate-600"}`}
                            >
                              {session.status}
                            </span>
                          </div>
...
// --- COMPONENT: ClassCard (Đã dọn dẹp phần enrolled) ---
